 ...
 * @param {String} code
 */
function OptimError(message, code) {
    this.message = message;
    this.code = code;
}

function _handleError(err, rawFile) {
    if (err.code === 'CANT_COMPRESS') {
        return { name: rawFile.name, exitCode: 1 };
    }

    return { name: rawFile.name, exitCode: 2 };
}

/**
 * Sequentially applies the specified algorithms to the given file
 * @param {File} rawFile
 ...
 */
function _runSequentially(rawFile, algorithms, reduceFunc, initVal) {
    return algorithms.reduce(reduceFunc, initVal)
        .fail(function () {
            throw new OptimError('Can not compress', 'CANT_COMPRESS');
        });
}
...
function _imageOptim(rawFile, algorithms, reduceFunc, initVal) {
    return rawFile.loadSize()
        .then(function () {
            return _runSequentially(rawFile, algorithms, reduceFunc, initVal);
        })
        .fail(function (err) {
            throw err;
        });
